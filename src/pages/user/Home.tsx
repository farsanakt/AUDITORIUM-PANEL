...
// Define interfaces locally if missing
interface Project {
  id: number;
  title: string;
  image: string;
...
const HomePage: React.FC = () => {
  ...
  const getSectionDescription = (type: string) => {
    if (type === "event management") {
      return "Expert planning for your perfect day."
    } else if (type === "makeup artist") {
      return "Enhance your beauty with professional artistry."
    } else if (type === "caterer") {
      return "Exquisite menus tailored to your taste."
    } else if (type === "decorator") {
      return "Stunning setups for memorable occasions."
    }
  ...
  const getFormattedPrice = (venue: Venue) => {
    if (!venue.price || venue.price === "Price not available") {
      return <span className="text-gray-500 italic text-sm">{venue.price}</span>
    }
      ...
      <section key={type} id={sectionId} className="py-12 w-full bg-white border-b border-gray-50 last:border-0">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full">
          <div className="flex justify-between items-end mb-6 animate-fade-in-up">
             <div>
                <h2 className="text-3xl font-bold text-[#5B4336] mb-1 font-serif tracking-tight">
             ...
             {vendors.length > 4 && (
                <div className="flex gap-2">
                   <button onClick={() => scrollLeft(type)} className="p-2 rounded-full border border-gray-200 text-gray-500 hover:border-[#9c7c5d] hover:text-[#9c7c5d] transition">
                      <ChevronLeft className="w-5 h-5" />
                   </button>
                   <button onClick={() => scrollRight(type)} className="p-2 rounded-full border border-gray-200 text-gray-500 hover:border-[#9c7c5d] hover:text-[#9c7c5d] transition">
          ...
          {vendorLoading ? (
             <div className="flex space-x-4 overflow-hidden py-2">
                {[1,2,3,4].map(i => (
                  <div key={i} className="min-w-[260px] h-[320px] bg-gray-100 rounded-lg animate-pulse skeleton" />
                ))}
             </div>
          ) : vendors.length === 0 ? (
            <div className="text-center text-gray-400 py-12 bg-gray-50/50 rounded-xl border border-dashed border-gray-200">No {type} vendors currently available.</div>
              ...
              <div
                ...
                className="scroll-container flex overflow-x-auto space-x-5 pb-6 snap-x snap-mandatory"
              >
                {vendors.map((artist, index) => (
                  <div
                    key={artist.id}
                    className="flex-shrink-0 w-[260px] sm:w-[280px] bg-white rounded-lg shadow-sm hover:shadow-xl transition-all duration-500 snap-center border border-gray-100 group cursor-pointer hover-lift flex flex-col justify-between"
                    onClick={() => navigate(`/vendordetails/${artist.id}`)}
                  ...
                  >
                    <div>
                      ...
                      <div className="p-4">
                        <h3 className="text-lg font-bold text-[#5B4336] mb-1 font-serif">{artist.name}</h3>
                        <p className="text-xs text-gray-500 flex items-center mb-0">
                          <MapPin className="h-3 w-3 mr-1 text-[#9c7c5d]" /> {artist.location}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="p-4 pt-0 mt-auto">
                      <div className="pt-3 border-t border-gray-50 flex justify-between items-center text-sm">
                        <div>
                           ...
                           <span className="text-[#9c7c5d] font-semibold">₹{artist.startingPrice}</span>
                        </div>
          ...
          <div className="text-center mt-4">
             <button
               ...
               className="text-[#9c7c5d] hover:text-[#8b6b4a] text-sm font-medium hover:underline transition underline-offset-4"
             >
               View all {getSectionTitle(type)}
             </button>
          </div>
        </div>
      </section>
  ...
  return (
    <div className="min-h-screen bg-white overflow-x-hidden w-screen relative font-sans text-gray-800">
      ...
      {/* Ad Banner Modal */}
     {showAd && (
      <div className="fixed inset-0 bg-black/40 z-[9999] flex items-center justify-center p-4 backdrop-blur-md transition-opacity duration-300">
        <div className="relative bg-white rounded-xl max-w-lg w-full shadow-2xl overflow-hidden animate-fade-in-up">
          <button
            onClick={closeAd}
            className="absolute top-4 right-4 z-20 bg-white/30 hover:bg-white/50 backdrop-blur p-2 rounded-full transition text-white"
          >
            <X className="w-5 h-5" />
          </button>
          ...
          <div className="relative h-56 w-full">
            ...
            <div className="absolute bottom-6 left-6 text-white">
              <span className="bg-[#ED695A] text-white px-3 py-1 rounded text-[10px] font-bold uppercase tracking-widest mb-2 inline-block">
                Limited Time
              </span>
          ...
          <div className="p-8 text-center">
            <h2 className="text-3xl font-bold text-[#5B4336] mb-2 font-serif">
              <span className="text-[#ED695A] mr-2">30% OFF</span>
              Venue Booking
            </h2>
            <p className="text-gray-500 mb-8 text-sm leading-relaxed">
              Book your dream venue today and save big on selected services. <br/> Don't miss out on this exclusive offer.
            </p>
            <button className="w-full bg-[#9c7c5d] hover:bg-[#8b6b4a] text-white font-semibold py-3.5 rounded-lg transition shadow-xl hover:shadow-2xl transform hover:-translate-y-0.5 text-sm uppercase tracking-wider">
              Claim Offer Now
            </button>
          </div>
        </div>
      ...
      <section className="relative min-h-[90vh] w-full overflow-hidden flex flex-col items-center justify-center">
          ...
          {heroImages.map((image, index) => (
            <div
              ...
              className={`absolute inset-0 w-full h-full bg-cover bg-center transition-opacity duration-[1500ms] ease-in-out ${
                index === heroCurrentSlide ? "opacity-100 scale-105" : "opacity-0 scale-100"
              }`}
        ...
        <div className="relative z-10 w-full max-w-5xl px-4 sm:px-6 flex flex-col items-center text-center pt-20">
          ...
          {currentUser && (
            <div className="animate-fade-in-up mb-6 px-6 py-2 glass rounded-full text-white font-medium text-xs uppercase tracking-widest border border-white/20">
               Welcome back, {displayName}
            </div>
          )}

          <div className="space-y-6 mb-12 animate-fade-in-up" style={{ animationDelay: '0.2s' }}>
            <h1 className="text-5xl md:text-7xl lg:text-8xl font-light text-white font-serif leading-none tracking-tight drop-shadow-lg">
              Unforgettable <br />
              <span className="font-normal italic text-[#e6d0b5] animate-pulse">Moments</span>
            </h1>
            <p className="text-white/90 text-lg font-light tracking-wide max-w-xl mx-auto drop-shadow-md">
              Discover curated venues and premium vendors for your perfect celebration.
            </p>
          </div>

          {/* CLASSIC SEARCH BAR (PILL SHAPE - OPAQUE GLASS) */}
          <div className="w-full max-w-4xl bg-white/80 backdrop-blur-md rounded-full p-2 shadow-2xl animate-fade-in-up border border-white/40 hidden md:flex items-center gap-2" style={{ animationDelay: '0.4s' }}>
              ...
              <div className="flex-1 relative pl-6 border-r border-gray-300/50">
               ...
               <div className="flex-1 relative px-6 border-r border-gray-300/50">
                  ...
                  <input
                    ...
                    className="w-full h-6 bg-transparent border-0 p-0 text-gray-900 text-sm font-medium focus:ring-0 placeholder-gray-600 cursor-pointer"
                  />
               </div>
               ...
               {/* Event Type Group */}
                <div className="flex-1 relative px-6">
                  ...
                  <select
                    ...
                    className="w-full h-6 bg-transparent border-0 p-0 text-gray-900 text-sm font-medium focus:ring-0 cursor-pointer appearance-none"
                  >
                    <option value="" className="text-gray-500">Add guests</option>
                ...
                <button
                    ...
                    className="w-14 h-14 bg-[#9c7c5d] hover:bg-[#8b6b4a] text-white rounded-full flex items-center justify-center transition-transform hover:scale-105 shadow-lg flex-shrink-0"
                  >
                     <Search className="w-6 h-6" />
                  </button>
          </div>

           {/* Mobile Search (Vertical Stack) */}
           <div className="w-full bg-white/90 backdrop-blur-md rounded-2xl p-4 shadow-xl md:hidden space-y-3 animate-fade-in-up border border-white/50">
              <LocationAutocomplete
                  value={formData.location}
              ...
              <div className="grid grid-cols-2 gap-3">
                 <input
                    ...
                    value={formData.date}
                    onChange={handleInputChange}
                    className="w-full h-12 bg-white/50 rounded-lg px-4 border border-gray-200 text-sm text-gray-700 focus:bg-white transition"
                  />
                  <select
                    name="event"
                    value={formData.event}
              ...
              <button
                  onClick={handleSubmit}
                  className="w-full h-12 bg-[#9c7c5d] text-white font-bold rounded-lg uppercase tracking-wider text-sm shadow-md"
               >
                   Search
               </button>
           </div>
            ...
            {loading ? (
             <div className="flex space-x-6 overflow-hidden py-4">
              ...
              <div
                ...
                ref={venuesRef}
                className="scroll-container flex overflow-x-auto space-x-6 pb-6 snap-x snap-mandatory px-2"
              >
        ...
        <section id="eventcategories" className="py-12 w-full bg-white overflow-hidden border-t border-gray-50">
           ...
           {vendorLoading ? (
            ...
            <div className="w-full relative">
              ...
              <div className="animate-marquee flex gap-16 items-center pl-8">
                 {/* Triplicating content to ensure no whitespace on load */}
                 {[
                      ...Object.entries(vendorsByType), 
                      ...Object.entries(vendorsByType), 
                      ...Object.entries(vendorsByType),
                      ...Object.entries(vendorsByType)
                    ].flatMap(([type, vendors], i) => 
                    vendors.slice(0, 1).map(vendor => (
                      ...
                      >
                         <div className="w-24 h-24 rounded-full overflow-hidden border-2 border-transparent group-hover:border-[#9c7c5d] p-1 transition-all duration-500">
                         ...
                         <h3 className="mt-4 font-bold text-gray-800 text-sm font-serif uppercase tracking-widest">{capitalizeWords(type)}</h3>
                      </div>
                    ))
                 )}
              </div>
      ...
      <Footer />
      {/* Search Modal for Terms, etc if needed */}
